...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 masking_strategy="random-masking",
    ...
    def random_jigsaw_masking(self, x, shuffle_ratio):
        """
        Perform jigsaw shuffling on a random subset of patches.
        ...
        # Jigsaw shuffle a random subset
        len_shuffle = int(L * shuffle_ratio)
        x_shuffled = x.clone()
        if len_shuffle > 1:
            for i in range(N):
                noise = torch.rand(L, device=x.device)
                ids_shuffle_subset = torch.argsort(noise)[:len_shuffle]
                new_order = torch.randperm(len_shuffle, device=x.device)
                shuffled_patches = x[i, ids_shuffle_subset, :][new_order]
                x_shuffled[i, ids_shuffle_subset, :] = shuffled_patches
        
        # No patches are masked, so the mask is all ones (loss is computed on all patches)
        mask = torch.ones([N, L], device=x.device)
    ...
    def keypoint_jigsaw_masking(self, x, keypoints):
        """
        Perform jigsaw shuffling on patches containing keypoints.
        No masking is applied. The full shuffled sequence is returned.
        The reconstruction target is the original, unshuffled patch sequence.
        """
        N, L, D = x.shape
        ...
        for i in range(N):
            ...
            if len(patch_indices_set) > 1:
                patch_indices = torch.tensor(list(patch_indices_set), device=x.device, dtype=torch.long)
    ...
    def forward_encoder(self, x, mask_ratio, shuffle_ratio, keypoints):
        ...
        if self.masking_strategy == "random-masking":
            x, mask, ids_restore = self.random_masking(x, mask_ratio)
        elif self.masking_strategy == "random-jigsaw":
    ...
    def forward(self, imgs, mask_ratio=0.75, shuffle_ratio=0.5, keypoints=None):
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio, shuffle_ratio, keypoints)
...
def mae_vit_base_patch16(**kwargs):
    model = MaskedAutoencoderViT(
        img_size=TARGET_SIZE, masking_strategy=MASKING_STRATEGY,
